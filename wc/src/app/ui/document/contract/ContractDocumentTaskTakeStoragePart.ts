...
import * as  contractDocumentTaskTakeStorageTemplate
    from "xhtmpl!app/ui/document/contract/templates/ContractDocumentTaskTakeStorageTemplate.hbs";
...
/**
 * Парт для приёма на хранение документа Договор.
 */
class ContractDocumentTaskTakeStoragePart extends ContractDocumentTaskBaseEditor {

    //TODO добавить resolver или builder для подобных имен
    /**
     * Имя парта.
     */
    static PART_NAME: string = "TaskPart:TaskType_Contract_TakeStorage:" + ContractDocument.meta.name;

    /**
     * Опции по умолчанию.
     */
    static DEFAULT_OPTIONS: ContractDocumentTaskTakeStoragePart.Options = {
        subtitle: resources["part.task.subtitle.contractTakeStorage"],
        mainPageOptions: {
            name: "main",
            title: resources["part.document.pages.main"],
            cssColumnPrefix: "col-xs-",
            template: contractDocumentTaskTakeStorageTemplate,
            properties: [
                ...
                {name: ContractDocument.meta.props.cost.name, readOnly: true},
                {name: ContractDocument.meta.props.currency.name, readOnly: true},
    ...
    /**
     * @constructs
     * @param {ContractDocumentTaskTakeStoragePart.Options} options Опции парта.
     */
    constructor(options?: ContractDocumentTaskTakeStoragePart.Options) {
        super(ContractDocumentTaskTakeStoragePart.mixOptions(options, ContractDocumentTaskTakeStoragePart.DEFAULT_OPTIONS));
    }
...
namespace ContractDocumentTaskTakeStoragePart {
    /**
...
core.createModule((app: Application) => {
    app.registerPart(ContractDocumentTaskTakeStoragePart.PART_NAME, (options?: ContractDocumentTaskTakeStoragePart.Options) => {
        return new ContractDocumentTaskTakeStoragePart(options);
    });
});
