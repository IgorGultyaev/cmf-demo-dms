package ru.croc.ctp.cmf.demodms.document.contract.impl.inboxitem;
...
import ru.croc.ctp.cmf.demodms.document.contract.domain.service.ContractDocumentService;
...
/**
 * Обработчик модели, создающий и обновляющий сущности {@link InboxItem} для переданной модели документа Договор.
 *
 * @author Andrei Dubonos
 * @since 2019.03.20
 */
@Component(ContractDocumentInboxItemHandler.NAME)
public class ContractDocumentInboxItemHandler implements InboxItemHandler<ContractDocument, String> {

    /**
     * Идентификатор бина для ссылки.
     */
    public static final String NAME = "ru.croc.ctp.cmf.dms.task.inboxitem.impl.ContractDocumentInboxItemHandler";
    ...
    /**
     * Репозиторий работы с документами Договор.
     */
    @Autowired
    ...
    @Override
    public void process(final ContractDocument model, final List<? extends DomainObject<?>> uow) {
        final String contractId = requireNonNull(model.getId());
        final InboxItem storedInboxItem = inboxItemRepository.findById(contractId).orElse(null);
     ...
     */
    boolean resolveHasAttachments(final ContractDocument model, final List<? extends DomainObject<?>> uow) {
